...
# Importações de terceiros (devem vir antes da configuração de ambiente)
from SigProfilerMatrixGenerator import install as genInstall
from SigProfilerMatrixGenerator.scripts import SigProfilerMatrixGeneratorFunc as matGen
from SigProfilerAssignment import Analyzer as Analyze
import sigProfilerPlotting as sigPlt

# --- CONFIGURAÇÃO DE AMBIENTE PARA SIGPROFILER (CRÍTICO) ---
# Define o diretório temporário para todos os dados do aplicativo
base_dir = Path("tmp") 
base_dir.mkdir(parents=True, exist_ok=True)

# Define onde os arquivos de referência (chrom_string, etc.) serão gravados.
# Isso resolve o erro [Errno 13] Permission denied em site-packages.
custom_refs_dir = base_dir / "SigProfilerMatrixGenerator" / "references" 
custom_refs_dir.mkdir(parents=True, exist_ok=True)

# Define a variável de ambiente globalmente para forçar o redirecionamento
# Isso deve ocorrer antes de qualquer chamada ou função que use o SigProfiler
...
def setup_logging(output_dir):
    ...
    logging.info(f"Variável de referência SigProfiler definida para: {os.environ.get('SIGPROFILER_REFERENCES_PATH')}")
    return log_file
...
def ensure_genome_installed(genome_build: str):
    ...
    logging.info(f"Tentando instalar o genoma {genome_build} no diretório: {os.environ.get('SIGPROFILER_REFERENCES_PATH')}")
...
def plot_contexts(base_dir, project, contexts):
    # base_dir aqui é o Path("tmp")
    for ctx in contexts:
        # Nota: O plotSBS gera arquivos no nível superior do 'project_dir' que é o "tmp/{project_name}"
        ...
        try:
            ...
            for file in valid_files:
                ...
                if file.name.endswith(".gz"):
                    # Descomprime arquivos .vcf.gz (não implementado neste trecho original, mas boa prática)
                    # O código original parece descompactar para o nome sem a extensão .gz
            ...
            # 🔹 Instalar Genoma (usará SIGPROFILER_REFERENCES_PATH)
            ...
            # Passa o 'base_dir' para o plot_contexts (que é "tmp")
...
# 🔹 Procurar resultados
# O fit_signatures gera resultados em output_dir, os plots em project_root
